...
export const KeyTakeawaysSlide: React.FC<SlideProps> = ({ slide }) => {
  const contentLines = (slide.content || []).map((line) => line.trim()).filter((line) => line.length > 0);
  const useContentAsItems = (!slide.items || slide.items.length === 0) && contentLines.length > 0;

  const items = (slide.items && slide.items.length > 0)
    ? slide.items
    : contentLines.map((line) => {
        const splitMatch = line.match(/^\s*(.+?)\s*(?:[:：—–-]\s+)\s*(.+)\s*$/);
        if (splitMatch) {
          return { title: splitMatch[1], description: splitMatch[2] };
        }
        return { title: line };
      });

  const bannerColor = slide.bottomBanner?.type === 'info'
    ? 'bg-blue-600'
    : slide.bottomBanner?.type === 'warning'
      ? 'bg-orange-500'
      : 'bg-red-600';
  ...
  return (
    <div className="flex flex-col h-full p-6 sm:p-8 md:p-12 lg:p-16 relative overflow-hidden bg-white">
      ...
      <div className="relative z-10 mb-8 sm:mb-12">
        ...
        {!useContentAsItems && slide.content && slide.content.length > 0 && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 pb-8">
          {items.map((item, idx) => (
            ...
            >
              ...
              <h3 className={`text-xl font-bold text-gray-800 group-hover:text-dify-blue transition-colors ${item.description ? 'mb-2' : 'mb-0'}`}>
      ...
      {slide.bottomBanner && (
        <div className={`absolute bottom-0 left-0 right-0 p-4 text-center text-white font-bold text-lg ${bannerColor} animate-slide-in-up`}>
